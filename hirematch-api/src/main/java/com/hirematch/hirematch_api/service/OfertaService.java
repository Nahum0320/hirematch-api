...
// Verificar que el usuario autenticado sea el propietario de la oferta
    ...
    public EstadisticasOfertaResponse obtenerEstadisticasOferta(Long ofertaId, Usuario usuario) {
        ...
        List<Pass> passes = passRepository.findByOferta(oferta);
        List<Chat> chats = chatRepository.findByOferta(oferta);
        ...
        response.setTasaRechazos(totalPostulaciones > 0 ?
            (double) (totalRechazosEmpresa + passes.size()) / totalPostulaciones : 0.0);
        response.setTasaRechazo(totalPostulaciones > 0 ?
            (double) passes.size() / totalPostulaciones : 0.0);
        response.setTasaContacto(totalMatches > 0 ? (double) totalContactados / totalMatches : 0.0);

        // Nivel experiencia promedio de postulantes (dejar en null por ahora)
        response.setNivelExperienciaPromedio(null);

        // Comparaciones y ranking (dejar en null por ahora)
        response.setRendimientoVsPromedio(null);
        response.setPosicionRanking(null);
